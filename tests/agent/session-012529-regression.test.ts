...
describe("session 012529 regression", () => {
  afterEach(async () => {
    await shutdownLsp();
  ...
  test("does not re-emit bootstrap required after probe success on non-applicable changes", async () => {
    tempDirectoryPath = await mkdtemp(join(tmpdir(), "zace-session-012529-"));
    const sessionId = `test-session-012529-${Math.random().toString(36).slice(2, 10)}`;
    ...
    const result = await runAgentLoop(llmClient, createTestConfig(), "create demo file and docs", {
      sessionId,
    });
    ...
    const probeSucceededIndex = runEvents.findIndex((event) => event === "lsp_bootstrap_probe_succeeded");
    expect(probeSucceededIndex).toBeGreaterThan(-1);
    const laterEvents = runEvents.slice(probeSucceededIndex + 1);
    expect(laterEvents).not.toContain("lsp_bootstrap_required");
  });
});
